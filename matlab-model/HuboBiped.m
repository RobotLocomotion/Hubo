classdef HuboBiped < TimeSteppingRigidBodyManipulator & Biped
  ...
  methods
    ...
    function obj = compile(obj)
      ...
      hubo_state_frame = HuboState(obj);
      tsmanip_state_frame = obj.getStateFrame();
      if isa(tsmanip_state_frame,'MultiCoordinateFrame')
        tsmanip_state_frame.frame{1} = hubo_state_frame;
        state_frame = tsmanip_state_frame;
      else
        state_frame = hubo_state_frame;
      end
      obj.manip = obj.manip.setStateFrame(hubo_state_frame);
      obj.manip = obj.manip.setOutputFrame(hubo_state_frame);
...
%     end
  ...
  properties (SetAccess = protected, GetAccess = public)
    x0
    default_footstep_params = struct('nom_forward_step', 0.07,... % m
                                      'max_forward_step', 0.3,...% m
                                      'max_step_width', 0.40,...% m
                                      'min_step_width', 0.10,...% m
                                      'nom_step_width', 0.20,...% m
