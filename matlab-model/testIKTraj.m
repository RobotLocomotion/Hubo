...
r = r.addRobotFromURDF('./urdf/hubo_minimal_contact.urdf',[],[],options);
warning(w);
nom_data = load('./data/hubo_fp.mat');
v = r.constructVisualizer();

r_foot = r.findLinkInd('Body_RightAnkleRoll');
l_foot = r.findLinkInd('Body_LeftAnkleRoll');
r_hand = r.findLinkInd('Body_RightWristPitch');
l_hand = r.findLinkInd('Body_LeftWristPitch');
head = r.findLinkInd('Body_Head');
pelvis = r.findLinkInd('Body_Hip');
...
l_leg_kny = find(strcmp(coords,'LeftKneePitch'));
r_leg_kny = find(strcmp(coords,'RightKneePitch'));
l_leg_hpy = find(strcmp(coords,'LeftHipPitch'));
r_leg_hpy = find(strcmp(coords,'RightHipPitch'));
l_leg_aky = find(strcmp(coords,'LeftAnklePitch'));
r_leg_aky = find(strcmp(coords,'RightAnklePitch'));
l_leg_hpz = find(strcmp(coords,'LeftHipYaw'));
r_leg_hpz = find(strcmp(coords,'RightHipYaw'));
...
kc3 = WorldPositionConstraint(r,r_hand,r_hand_pts,r_hand_pos+[0.25;0.05;0.3],r_hand_pos+[0.45;0.05;0.4],[tspan(end) tspan(end)]);
...
pc_knee = pc_knee.setJointLimits([l_leg_kny;r_leg_kny],[0.1;0.1],[inf;inf]);
...
cost.base_pitch = 100;
cost.base_yaw = 100;
cost.LeftElbowPitch = 0.01;
cost.RightElbowPitch = 0.01;
cost = double(cost);
Q = diag(cost(1:nq));
ikoptions = ikoptions.setQ(Q);
ikoptions = ikoptions.setQa(0.001*Q);
ikoptions = ikoptions.setMajorIterationsLimit(50000);
...
% display('Constraining that pelvis shouldn''t move');
