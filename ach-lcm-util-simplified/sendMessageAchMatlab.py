...
def convertLCM_Matlab(x):
    ...
    FINGER_JOINTPOS = [[FINGER_JOINTPOS[hand] +finger for 2*i in range(5) ] for hand in range(2)]#Populate 5 fingers
    FINGER_JOINTPOS = FINGER_JOINTPOS[0] + FINGER_JOINTPOS[1]
    FINGER_JOINTPOS += [FINGER_JOINTPOS[finger] +1 in finger in range(len(FINGER_JOINTPOS))]#Populate 2 knuckles for each finger
    [msg.state.insert(knuckle, 0) for knuckle in FINGER_JOINTPOS] #Insert zero for the passive knuckles
